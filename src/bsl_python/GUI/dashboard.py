...
def change_electrode(electrode, file):
    ...
    if file != dashboard.nwb_files[dashboard.c_file]["value"]:
        dashboard.c_file = [file_index for file_index in range(len(dashboard.nwb_files)) if
                            dashboard.nwb_files[file_index]["value"] == file][0]
        nwb_io = NWBHDF5IO(dashboard.nwb_files[dashboard.c_file]["fullpath"], 'r')
        nwb_file = nwb_io.read()
        try:
    ...
    print("Load data --- %s seconds ---" % (toc - tic))
    if error is not None:
        return dash.no_update, dash.no_update, dash.no_update, True, [html.P("{0}".format(error), className="mb-0")]
